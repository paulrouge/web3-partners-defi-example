from icon_utils.iconcalls import print_latest_block, print_transactions_from_block, print_balanced_price



# print_latest_block()
# print_transactions_from_block(63762860, clean_output=False)

# pick pool id from the contract (see tracker link below)
# print_balanced_price(3)







# hex to utf8
# hexstring = '0x7b226d6574686f64223a225f7374616b65227d'
# print(bytes.fromhex(hexstring[2:]).decode('utf-8'))

# just a helper, the url to the balanced dex contract address
# https://tracker.icon.community/contract/cxa0af3165c08318e988cb30993b3048335b94af6c#transactions

